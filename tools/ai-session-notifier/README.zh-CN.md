...
**AI Session Notifier** 会在 Codex 或 Claude Code 停下、空闲或等待权限时显示桌面提醒，并尽力返回原来的应用、工作区和会话；其 Codex 适配器以 `codex-notify-hooks` 插件形式提供。
...
| 返回会话 | 根据 hook 元数据自动选择 Codex 桌面端、VS Code 或 Claude Code。 |
...
需要特别说明：Codex 的 `Stop` 只表示**当前这一轮停下**，不等于整个任务已完成；Claude Code 的 `idle_prompt` 也只表示当前会话正在等待。

## 安装

clone 仓库并进入仓库根目录，然后按本机需要选择适配器。

### macOS：同时安装 Codex 与 Claude Code
...
## 给 Codex / Claude Code 等 Agent 的安装区
...
2. 检测操作系统和用户要求的适配端。Codex 仅支持 macOS；Claude Code 支持 macOS、Linux、Windows。不要声称已经在 Linux/Windows 安装 Codex 适配器。
3. macOS 安装前先预览：
...
4. 根据用户要求运行 `--all`、`--codex` 或 `--claude`。只有用户明确同意修改包管理器时，才可添加 `--install-deps`。
5. Linux/Windows 使用上面的 Claude CLI 命令安装或更新 `ai-session-notifier@dylon-tiny-kits`，不要手工修改 Claude 插件缓存。
6. 不要直接覆盖 `~/.codex/hooks.json`。必须使用安装器，让它备份并合并本工具自己的 hook；不要使用 `sudo`。
7. 安装后默认执行无 UI 验收：
...
8. 除非用户明确接受隐私取舍，否则不要开启消息摘要、原始 payload 或调试日志。安装和排障过程中绝不能运行卸载命令的 `--purge`。
9. 完成后向用户报告：安装了哪些适配器、版本、修改路径、`doctor` 结果和剩余警告。提醒用户 Claude Code 更新后需要重启；Codex hook 变化后可能需要在新任务中审查并信任 hook。
...
`sessions.json` 只保存每个会话最新的一条路由。通知触发时，适配器根据来源自动选择 Codex 桌面端、VS Code 或 Claude Code，不再让用户手动判断“这个任务当时在哪个 App”。
...
Codex 桌面端使用 `codex://threads/<thread_id>`；Claude Code 使用 `claude-cli://open` 目录链接。Windows 的 Claude 适配器包含按窗口标题匹配的 VS Code 聚焦逻辑。
...
| --- | --- | --- |
| macOS | 已实测。支持 `terminal-notifier`/AppleScript、Codex App 与 VS Code 路由、多窗口尽力定位。 | 已实测。支持 `terminal-notifier`/AppleScript 和 `claude-cli://`。 |
| Linux | 暂未封装。 | 已包含 `notify-send` 适配器并通过自动测试，仍需要更多桌面环境实测。 |
| Windows | 暂未封装。 | 已包含 PowerShell 对话框、加锁事件记录、自动清理和 VS Code 尽力聚焦；有自动检查，仍需要真机体验验证。 |

macOS 会优先读取本机 ChatGPT/Claude 应用中的图标，找不到时再读取 OpenAI/Anthropic 官方 VS Code 扩展中的图标。也可以为两个工具分别指定本地图标；仓库本身不再分发厂商 Logo 文件。
...
- `notifications.codexIconPath`、`notifications.claudeIconPath`：分别覆盖两个工具的本地图标。
- `notifications.iconPath`：两个工具共用的兜底图标。
...
Codex 卸载器只移除本工具写入的 `Stop`、`PermissionRequest` 项，不会删除其他 hooks；安装器修改共享配置前会先备份。
...
测试覆盖默认脱敏、私有权限、保留期、旧数据迁移、并发写入、安装后命令查找，以及安装/卸载时保留无关 Codex hooks。GitHub Actions 会在 macOS、Linux、Windows 上运行相应检查。

Codex、Claude 等名称与商标归各自权利人所有。本工具是社区项目，与 OpenAI、Anthropic 无隶属或背书关系。
