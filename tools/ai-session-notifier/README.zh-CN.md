...
| 降噪 | 合并短时间重复提醒，识别观察窗口，并静音由 Codex“替我审批”处理的权限事件。 |
...
`noise.permissionMode` 默认为 `smart`。Codex 的 `PermissionRequest` 在当前版本里没有直接区分人工审批和“替我审批”，因此适配器会优先读取 hook 将来可能提供的 reviewer 字段；字段缺失时，只从当前任务的本地 transcript 中反向查找最新结构化 `approvals_reviewer` 状态。

- 明确为 `auto_review`、兼容旧名 `guardian_subagent`，或 `bypassPermissions` 时，只记录事件，不显示弹窗、横幅或声音。
- 明确为 `user`、无法读取或遇到未知值时继续提醒，防止漏掉真正卡住的权限请求。
- 读取过程不会保存 transcript 消息正文；事件账本只记录 reviewer/mode 枚举和抑制原因。

也可以将 `noise.permissionMode` 改为 `notify`（任何 Codex 权限事件都提醒）或 `quiet`（任何 Codex 权限事件都静音）。环境变量 `AI_SESSION_NOTIFIER_PERMISSION_MODE` 可临时覆盖该设置。

Claude Code 的 `Notification: permission_prompt` 和 Kimi Code 的 `PermissionRequest` 都表示即将真正等待用户。它们的自动审批通常不会产生这类提醒；如果自动模式最终仍触发事件，往往就是需要人工处理的回退，因此这两个适配器有意保持提醒。
...
- `noise.permissionMode`：Codex 权限提醒使用 `smart`（默认）、`notify` 或 `quiet`。
...
环境变量 `AI_SESSION_NOTIFIER_LOCALE` 和 `AI_SESSION_NOTIFIER_PERMISSION_MODE` 的优先级高于配置文件，可在单个进程或 shell 环境中临时覆盖对应设置。
...
测试覆盖默认脱敏、私有权限、保留期、旧数据迁移、Codex 智能审批降噪、Claude/Kimi 人工回退提醒、Kimi 事件语义、并发写入、安装后命令查找，以及安装/卸载时保留无关 Codex hooks。GitHub Actions 会在 macOS、Linux、Windows 上运行相应检查。
